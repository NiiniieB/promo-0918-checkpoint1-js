...
const readAnswers = () => fs.readFileAsync(`${__dirname}/answers.txt`)
...
const checkAnswers = branchName => () => readAnswers()
  ...
  .catch(err => {
    console.error(err);
    throw new Error(`On branch ${branchName}: ${err.message}`);
  })
...
.then(currentBranch => runAllBranches()
  .then(results => checkoutBranch(currentBranch)
    .then(() => {
      ...
      console.log(json)
      return fs.writeFileAsync(`${__dirname}/results.json`, json)
    })
    .then(() => {
      console.log('done')
      process.exit()
  ...
  // .then(() => {
  //   results.forEach(({ branchName, tests}) => {
  //     console.log(branchName)
  //     console.log(tests.statuses)
  //   })
  // })
  ...
  .catch(console.error)
// .then(html => fs.writeFileAsync('results.html', html))
